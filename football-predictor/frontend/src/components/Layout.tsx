  ...
  return (
    <span className="flex items-center gap-2 select-none">
      ...
      >
        ...
        />
        <polygon
          points="14,7 18,11 16,17 12,17 10,11"
          fill="#fff"
          stroke="#1e40af"
          strokeWidth="1.2"
        />
      ...
      <span className="font-extrabold text-xl tracking-tight text-primary">
        Match<span className="text-accent">Wise</span>
      </span>
    </span>
  );
}
...
function UserAvatar() {
  ...
  return (
    <div className="ml-6 flex items-center gap-2 cursor-pointer group">
      <img
        src="https://ui-avatars.com/api/?name=User&background=2563eb&color=fff&rounded=true&size=32"
        alt="User"
        className="w-8 h-8 rounded-full border-2 border-primary/30 group-hover:border-primary transition"
      />
      <span className="hidden md:inline text-sm font-semibold text-text-secondary group-hover:text-primary transition">
        Profile
      </span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen w-screen bg-gradient-to-br from-background-DEFAULT to-background-soft overflow-x-hidden font-sans">
      ...
      <nav className="sticky top-0 left-0 right-0 h-16 bg-white/95 border-b border-slate-200 flex items-center px-6 z-30 w-full shadow-sm backdrop-blur-md">
        {/* Logo */}
        <button
          className="flex items-center gap-2 mr-10 focus:outline-none"
          onClick={() => navigate("/")}
