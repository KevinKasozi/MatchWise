...
export default function Fixtures() {
  ...
  const { data: fixturesData = [], isError } = useQuery<
    (Fixture & { result?: MatchResult })[]
  >({
    queryKey: ["fixtures", status],
    queryFn: async () => {
      const res = await apiClient.get<(Fixture & { result?: MatchResult })[]>(
        "/fixtures",
        {
  ...
  return (
    <div>
      ...
      <Tab.Group
        selectedIndex={tabOptions.findIndex((t) => t.value === status)}
        onChange={(i) => setStatus(tabOptions[i].value)}
      >
                      ...
                      .map(([date, fixtures]) => {
                        ...
                        fixtures.forEach((fixture) => {
                          const competitionKey =
                            fixture.competition_name || "Unknown Competition";
                        ...
                        return (
                          ...
                          >
                            ...
                            {Object.entries(fixturesByCompetition).map(
                              ([competition, competitionFixtures]) => (
