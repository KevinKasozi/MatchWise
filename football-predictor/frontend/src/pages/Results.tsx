          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {filtered.map((r) => (
                <tr
                  key={r.id}
                  className="border-b last:border-0 hover:bg-primary/10 transition duration-150 cursor-pointer"
                >
                  ...
                  <td className="p-2">
                    <span
                      className={`font-bold ${r.prediction === "Draw" ? "text-slate-700" : r.prediction === r.home.name ? "text-primary" : "text-error"}`}
                    >
                      {r.prediction}
                    </span>
                  </td>
        ...
        <div className="space-y-2">
          {accuracyByLeague.map((l, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="w-32 text-slate-900 font-semibold">
                {l.league}
              </span>
              <div className="flex-1 h-4 bg-slate-200 rounded overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${l.accuracy * 100}%`,
                    transition: "width 0.5s",
                  }}
                />
              </div>
              <span className="w-10 text-right font-mono text-primary">
