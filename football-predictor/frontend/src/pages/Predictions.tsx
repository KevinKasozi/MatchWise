  ...
  return (
    <div className="w-full p-4 space-y-8">
      ...
      <p className="text-gray-600 mb-6">
        Our AI model uses historical match data and team statistics to predict match outcomes.
      </p>
      ...
      <div className="bg-white/60 backdrop-blur-md shadow-glass rounded-2xl p-8 border border-primary-light">
        ...
        <div className="flex items-center gap-4 mb-4">
          <img src={match.home.crest} alt={match.home.name} className="w-10 h-10 object-contain" />
          ...
          <img src={match.away.crest} alt={match.away.name} className="w-10 h-10 object-contain" />
        </div>
        
        <div className="text-sm text-slate-500 mb-4">
          {match.competition} • {new Date(match.match_date).toLocaleDateString()} • {match.kickoff}
        </div>
        
        {/* Predicted Score */}
        <div className="mb-4 text-center">
          <div className="text-xs uppercase tracking-widest text-slate-700 mb-2">Predicted Score</div>
          <div className="flex items-center justify-center gap-2">
            <span className="font-bold text-2xl" style={{ color: match.home.color }}>{match.prediction.home_score}</span>
            <span className="text-slate-400">-</span>
            <span className="font-bold text-2xl" style={{ color: match.away.color }}>{match.prediction.away_score}</span>
          </div>
        </div>
        ...
        <div className="text-sm text-slate-600 mb-4 text-center">
          Prediction: <span className="font-bold text-primary">{match.prediction.winner}</span>
        </div>
        
        {/* Features */}
        <div className="mt-6">
          <div className="text-xs uppercase tracking-widest text-slate-700 mb-2">Key Factors</div>
