...
const team = {
  ...
  results: [
    { id: 1, opponent: 'Liverpool', date: '2024-05-03', score: '2-1', home: true, correct: true },
    { id: 2, opponent: 'Tottenham', date: '2024-04-27', score: '1-2', home: false, correct: false },
  ],
  accuracy: 0.74,
};
    ...
    <div className="w-full px-4 md:px-8 py-10 max-w-screen-lg mx-auto grid gap-10">
      ...
      <div className="flex items-center gap-6 bg-gradient-to-br from-surface-alt to-surface/80 rounded-2xl shadow-2xl p-10 border-l-8 border-primary/80 hover:scale-[1.01] hover:shadow-2xl transition-transform duration-200">
        ...
        <div className="flex-1">
          <div className="text-2xl font-bold text-primary mb-1">{team.name}</div>
          <div className="text-slate-700 text-sm mb-1">Stadium: <span className="font-semibold">{team.stadium}</span></div>
          <div className="text-slate-700 text-sm mb-1">Coach: <span className="font-semibold">{team.coach}</span></div>
          <div className="text-slate-700 text-sm">Country: <span className="font-semibold">{team.country}</span></div>
        </div>
        <button
      ...
      {tab === 0 && (
        <div className="bg-gradient-to-br from-surface-alt to-surface/80 rounded-2xl shadow-lg p-8 border-l-4 border-accent/70 hover:shadow-2xl transition-shadow">
          <div className="text-xs uppercase tracking-widest text-slate-700 mb-2 font-semibold">Upcoming Fixtures</div>
      ...
      {tab === 1 && (
        <div className="bg-gradient-to-br from-surface-alt to-surface/80 rounded-2xl shadow-lg p-8 border-l-4 border-secondary/70 hover:shadow-2xl transition-shadow">
          <div className="text-xs uppercase tracking-widest text-slate-700 mb-2 font-semibold">Recent Results</div>
          <ul className="divide-y divide-slate-200">
            {team.results.map((r) => (
              <li key={r.id} className="py-2 flex items-center gap-4">
                ...
                {r.correct ? (
                  <span className="inline-flex items-center gap-1 text-success font-semibold">✅</span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-error font-semibold">❌</span>
                )}
              </li>
            ))}
          </ul>
      ...
      {tab === 2 && (
        <div className="bg-gradient-to-br from-surface-alt to-surface/80 rounded-2xl shadow-lg p-8 border-l-4 border-success/70 hover:shadow-2xl transition-shadow">
          <div className="text-xs uppercase tracking-widest text-slate-700 mb-2 font-semibold">Prediction Accuracy</div>
          {/* Form Line Graph (mocked) */}
          <div className="w-full h-32 flex items-end gap-1 mb-4">
            {team.form.map((f, i) => (
              <div key={i} className={`flex-1 rounded-t ${f === 'W' ? 'bg-success' : f === 'D' ? 'bg-warning' : 'bg-error'}`} style={{ height: `${30 + i * 5}px` }} />
            ))}
          </div>
          <div className="text-slate-700 text-sm mb-2">Last 10 matches: <span className="font-semibold">{team.form.join(' ')}</span></div>
          <div className="text-slate-700 text-sm">Model accuracy: <span className="font-semibold text-primary">{Math.round(team.accuracy * 100)}%</span></div>
        </div>
      )}
    </div>
  );
